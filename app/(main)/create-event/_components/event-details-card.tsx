            ...
            <CardContent className="space-y-8 pt-8 px-6 md:px-8">
                ...
                <div className="space-y-3">
                    ...
                    <div className="relative">
                        <Input
                            ...
                            className="bg-black/40 border-white/10 h-12 text-lg focus:border-emerald-500/50 focus:ring-emerald-500/20 rounded-xl transition-all pl-4"
                        />
                        <div className="absolute inset-0 rounded-xl bg-gradient-to-r from-emerald-500/10 to-transparent pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity" />
                    </div>
                    {errors.title && (
                    ...
                    <Controller
                        ...
                        render={({ field }) => (
                            <Select value={field.value} onValueChange={field.onChange}>
                                <SelectTrigger className="w-full bg-black/40 border-white/10 h-12 rounded-xl focus:ring-emerald-500/20">
                                ...
                                <SelectContent className="bg-zinc-900 border-white/10 max-h-[300px]">
                                    {CATEGORIES.map((cat) => (
                                        <SelectItem key={cat.id} value={cat.id} className="focus:bg-emerald-500/20 focus:text-emerald-100 cursor-pointer py-3">
                                            <span className="flex items-center gap-3">
                                                <div className="p-1 rounded bg-white/5">
                                                    <CategoryIcon name={cat.icon} className="w-4 h-4 shrink-0 text-emerald-400" />
                                                </div>
                ...
                <div className="space-y-3">
                    ...
                    <Textarea
                        ...
                        rows={6}
                        className="bg-black/40 border-white/10 focus:border-emerald-500/50 rounded-xl resize-none p-4 leading-relaxed scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent"
                    />
                    {errors.description && (
                        <p className="text-sm text-red-400 mt-1 flex items-center gap-1">
                            <span className="w-1 h-1 rounded-full bg-red-400" />
